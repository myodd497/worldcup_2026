...
def _system_prompt() -> str:
    return (
        ...
        "\n"
        "### Q3: Top players by goal contributions (goals + assists) in last 10 games\n"
        "Tables: `fact_match_team` (to find last 10 match_ids) + `fact_match_event` (for goals/assists).\n"
        "Pattern: CTE for last 10 match_ids → CTE for goals (WHERE is_goal=TRUE, GROUP BY player_name) → CTE for assists (WHERE is_goal=TRUE AND assist_name IS NOT NULL, GROUP BY assist_name) → FULL OUTER JOIN goals and assists on player_name → COALESCE(goals,0) + COALESCE(assists,0) AS goal_contributions → ORDER BY DESC LIMIT 10.\n"
        "ⓘ `is_goal` already excludes Missed Penalty (event_type='Goal' but event_detail='Missed Penalty'). If querying event_type/event_detail directly, always add `AND event_detail != 'Missed Penalty'`.\n"
        "ⓘ `fact_match_event` only has events for matches where API-Football provides event data — friendlies may be missing.\n"
        "\n"
        "### Q4: Top players by worst discipline (yellow + red cards) in last 10 games\n"
        "Tables: `fact_match_team` + `fact_match_event`.\n"
        "Pattern: CTE for last 10 match_ids → SELECT player_name, COUNTIF(is_yellow_card), COUNTIF(is_red_card), COUNTIF(is_yellow_card) + 2*COUNTIF(is_red_card) AS discipline_score → WHERE is_yellow_card OR is_red_card → GROUP BY player_name → ORDER BY discipline_score DESC LIMIT 10.\n"
        "\n"
        "### Q5: Most minutes played\n"
        "ⓘ **Cannot answer accurately.** The data model does not have a `minutes_played` column. `fact_match_event` tracks goals/cards/substitutions but not cumulative minutes. This requires a new raw data source (API-Football's player statistics endpoint).\n"
        "Best available proxy: COUNT(DISTINCT match_id) AS matches_appeared, COUNT(*) AS events_involved from `fact_match_event` — but this is NOT true minutes.\n"
        "\n"
