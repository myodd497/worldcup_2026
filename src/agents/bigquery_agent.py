...
def _system_prompt() -> str:
    return (
        ...
        "**Primary table:** `fact_player_match_stat`.\n"
        "Pattern: SELECT fp.player_id, dp.player_name, ROUND(AVG(fp.rating), 2) AS avg_rating, COUNT(*) AS matches FROM fact_player_match_stat fp JOIN dim_player dp USING (player_id) WHERE fp.competition_id = 1 AND fp.season_year = 2026 AND fp.rating IS NOT NULL GROUP BY fp.player_id, dp.player_name HAVING COUNT(*) >= 2 ORDER BY avg_rating DESC LIMIT 10.\n"
